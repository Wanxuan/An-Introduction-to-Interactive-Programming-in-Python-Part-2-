...
def draw(canvas):
    ...
    canvas.draw_text("Lives: "+str(lives), (WIDTH/15, HEIGHT/10), 30, "pink")
    canvas.draw_text("Score: "+str(score), (WIDTH*0.8, HEIGHT/10), 30, "pink")
...
def rock_spawner():
    ...
    a_rock = Sprite([random.randrange(WIDTH), random.randrange(HEIGHT)],
                    [random.choice([-1, 1])*random.random()*2, random.choice([-1, 1])*random.random()*2], 
                    1, 3.14 / (random.choice([-1, 1])*random.randint(100, 150)),
...
a_rock = Sprite([WIDTH / 3, HEIGHT / 3], [1, 1], 0, 3.14/50, asteroid_image, asteroid_info)
